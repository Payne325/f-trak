        ...
        loop {
            ...
            if success {
                ...
                for detection_index in 0..num_detections {
                    ...
                    let raw_start_x = detections.at_nd::<i32>(&start_x_index).unwrap();
                    let raw_start_y = detections.at_nd::<i32>(&start_y_index).unwrap();
                    let raw_end_x = detections.at_nd::<i32>(&end_x_index).unwrap();
                    let raw_end_y = detections.at_nd::<i32>(&end_y_index).unwrap();

                    let start_x = raw_start_x * frame_size.width;
                    let start_y = raw_start_y * frame_size.height;
                    let end_x = raw_end_x * frame_size.width;
                    let end_y = raw_end_y * frame_size.height;

                    // I used to know why this was required... Made this
                    // constant look super dirty to encourage me to revisit
                    // this.
                    const MAGIC_Y_OFFSET: i32 = 10;
                    let start_y = if start_y.wrapping_sub(MAGIC_Y_OFFSET) > MAGIC_Y_OFFSET {
                        start_y - MAGIC_Y_OFFSET
                    } else {
                        start_y + MAGIC_Y_OFFSET
                    };

                    let start_pt = opencv::core::Point2i::new(start_x, start_y);
                    let end_pt = opencv::core::Point2i::new(end_x, end_y);
